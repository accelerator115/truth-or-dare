<template>
	<view class="container">		<view class="mode-selector card">
			<view class="title">选择模式</view>
			<view class="mode-buttons">
				<view class="mode-btn" :class="{'active': selectedMode === 'truth'}" @click.stop="selectMode('truth')">真心话</view>
				<view class="mode-btn" :class="{'active': selectedMode === 'dare'}" @click.stop="selectMode('dare')">大冒险</view>
			</view>
		</view>
		...
		<view class="card">
			...
			<view class="import-btn" @click.stop="importDefaultItems">导入默认内容</view>
				...
				<view class="item" v-for="(item, index) in items" :key="index" :class="{'current-mode': item.type === selectedMode}">					<view class="item-content">
				...
				<view class="wheel-section card" v-if="filteredItems.length >= 3">		<wheel 
			:items="filteredItems" 
			:spinning="isSpinning" 
	...
	<view class="card notice" v-else>
		...
		<view class="history-list">
			<view class="history-item" v-for="(item, index) in history" :key="index">
				...
				<view class="history-content">{{item.text}}</view>			</view>
		</view>
...
<style>
	...
	.spin-btn:active, .stop-btn:active {
		opacity: 0.8;
		transform: scale(0.98);
	}
